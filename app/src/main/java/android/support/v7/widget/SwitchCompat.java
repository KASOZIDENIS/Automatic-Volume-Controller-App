...
public class SwitchCompat extends CompoundButton {
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        TypedArray appearance = context.obtainStyledAttributes(resid, R.styleable.SwitchCompatTextAppearance);
        ColorStateList colors = appearance.getColorStateList(R.styleable.SwitchCompatTextAppearance_android_textColor);
        if (colors != null) {
            this.mTextColors = colors;
        } else {
            this.mTextColors = getTextColors();
        }
        int ts = appearance.getDimensionPixelSize(R.styleable.SwitchCompatTextAppearance_android_textSize, 0);
        if (!(ts == 0 || ((float) ts) == this.mTextPaint.getTextSize())) {
            this.mTextPaint.setTextSize((float) ts);
            requestLayout();
        }
        if (appearance.getBoolean(R.styleable.SwitchCompatTextAppearance_textAllCaps, false)) {
    ...
    private Layout makeLayout(CharSequence text) {
        ...
        return new StaticLayout(transformed, this.mTextPaint, (int) Math.ceil((double) Layout.getDesiredWidth(transformed, this.mTextPaint)), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (switchText != null) {
            ...
            canvas.translate((float) left, (float) (((switchInnerTop + switchInnerBottom) / 2) - (switchText.getHeight() / 2)));
